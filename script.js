...
let Playback = {
  ...
  updateUI() {
    ...
    if(typeof renderPlaybackState === 'function') renderPlaybackState(this.tick, this.segs, this.procs, this.maxTick);
  }
};
...
function switchMode(m){ 
  mode=m; 
  document.querySelectorAll('.mode-tab').forEach((t,i)=>t.classList.toggle('active',['single','compare'][i]===m)); 
...
function addProc(){ 
  const a=+document.getElementById('arr').value,b=+document.getElementById('burst').value; 
  const p=+document.getElementById('pri').value||1; 
  const err=document.getElementById('err'); 
  if(b<1){err.textContent='Burst must be ≥ 1';return} 
  err.textContent='';procs.push({id:pid++,arr:a,burst:b,pri:p});renderTags(); 
} 
function randomProcs(){ 
  procs=[];pid=1; 
  const n=Math.floor(Math.random()*4)+3; 
  for(let i=0;i<n;i++) procs.push({id:pid++,arr:Math.floor(Math.random()*6),burst:Math.floor(Math.random()*8)+1,pri:Math.floor(Math.random()*5)+1}); 
  renderTags(); 
} 
function removeP(i){procs.splice(i,1);renderTags()} 
function clearAll(){procs=[];pid=1;renderTags();document.getElementById('output').classList.add('hidden');document.getElementById('empty').style.display='flex';} 
function renderTags(){ 
  ...
  pl.innerHTML=procs.map((p,i)=>`<div class="tag"><div class="tag-info"><div class="tdot" style="background:${COLORS[i%COLORS.length]}"></div><span>P${p.id} a:${p.arr} b:${p.burst}${showPri?` pr:${p.pri}`:''}</span></div><span class="tag-x" onclick="removeP(${i})">×</span></div>`).join(''); 
} 
...
function addIdle(res,from,to){if(from<to)res.push({id:'idle',start:from,end:to});} 
...
function hrrn(ps){ 
  ...
  while(done.size<ps.length){ 
    let av=sorted.filter(p=>!done.has(p.id)&&p.arr<=t); 
    if(!av.length){const nx=sorted.find(p=>!done.has(p.id));addIdle(res,t,nx.arr);t=nx.arr;continue;} 
    // Response ratio = (waiting time + burst) / burst 
    let p=av.sort((a,b)=>{ 
...
// Q1 (burst 1-3): FCFS, Q2 (burst 4-6): RR(q=2), Q3 (burst 7+): SJF 
function multiQueue(ps){ 
  ...
  // Q1 runs first (highest priority), then Q2, then Q3 
  let t=0,res=[]; 
  function runFCFS(group){ 
    [...group].sort((a,b)=>a.arr-b.arr).forEach(p=>{ 
      t=Math.max(t,p.arr);addIdle(res,res.length?res[res.length-1].end:0,t); 
      // remove consecutive idle duplicates - just push 
...
function mfq(ps){ 
  ...
  function enq(now){ 
    procs2.forEach(p=>{ 
      if(!arrived.has(p.id)&&p.arr<=now){arrived.add(p.id);q1.push(p);} 
    }); 
  } 
  enq(0); 
  let safety=0; 
...
function computeStats(segs,ps){ 
  let finish={},start={}; 
  segs.filter(s=>s.id!=='idle').forEach(s=>{if(start[s.id]===undefined)start[s.id]=s.start;finish[s.id]=s.end;}); 
  return ps.map(p=>({id:p.id,arr:p.arr,burst:p.burst,pri:p.pri,finish:finish[p.id]??0,wt:(start[p.id]??p.arr)-p.arr,tat:(finish[p.id]??p.arr)-p.arr})); 
} 
function ci(id){return procs.findIndex(p=>p.id===id)} 
function buildLegend(stats){ 
  const mw=Math.max(...stats.map(p=>p.wt)); 
  return`<div class="legend">${procs.map((p,i)=>{const s=stats.find(x=>x.id===p.id);const lw=s&&s.wt===mw&&mw>0;return`<div class="li${lw?' lw':''}"><div class="ld" style="background:${COLORS[i%COLORS.length]}"></div>P${p.id} wt:${s?s.wt:0}ms${lw?'<span class="wbadge">longest</span>':''}</div>`;}).join('')}</div>`; 
} 
function buildGantt(segs,total,gid,h=48){ 
  const bars=segs.map(s=>{ 
    ...
    return`<div class="gb${s.id==='idle'?' idle':''}" data-w="${w}" style="background:${color};height:${h}px" title="${s.id==='idle'?'IDLE':`P${s.id}`}: ${s.start}–${s.end}${lvl}">${s.id==='idle'?'·':`P${s.id}`}</div>`; 
  }).join(''); 
  const ticks=[...new Set(segs.flatMap(s=>[s.start,s.end]))].sort((a,b)=>a-b); 
  const tr=ticks.map((t,i)=>{const nx=ticks[i+1];const w=nx?(((nx-t)/total)*100).toFixed(3):'0';return`<div class="tick" style="width:${w}%;min-width:${i===ticks.length-1?'14px':'0'}">${t}</div>`;}).join(''); 
...
function buildStatCards(stats){ 
  ...
  return`<div class="stat-row"><div class="sc"><div class="sc-label">Avg Waiting</div><div class="sc-val">${aw}<span>ms</span></div></div><div class="sc"><div class="sc-label">Avg Turnaround</div><div class="sc-val">${at}<span>ms</span></div></div><div class="sc"><div class="sc-label">Total Time</div><div class="sc-val">${tot}<span>ms</span></div></div></div>`; 
} 
function buildTable(stats,showPri=false){ 
  ...
  return`<div class="twrap"><table><thead><tr><th>Process</th><th>Arrival</th><th>Burst</th>${priCol}<th>Finish</th><th>Waiting</th><th>Turnaround</th></tr></thead><tbody>${stats.map(p=>`<tr><td class="tpid">P${p.id}</td><td>${p.arr}</td><td>${p.burst}</td>${priCells(p)}<td>${p.finish}</td><td class="${p.wt===mw&&mw>0?'hw':p.wt===lw?'lw2':''}">${p.wt}</td><td>${p.tat}</td></tr>`).join('')}</tbody></table></div>`; 
} 
...
function buildQChart(ps,curQ){ 
  if(!ps.length)return''; 
  const pts=[]; 
  for(let q=1;q<=10;q++){const segs=rrAlgo(ps,q);const st=computeStats(segs,ps);pts.push({q,aw:+(st.reduce((s,p)=>s+p.wt,0)/st.length).toFixed(2)});} 
  ...
  return`<div class="qchart-panel"><div class="qchart-title">// quantum impact — avg waiting time vs quantum (Round Robin)</div><svg viewBox="0 0 ${W} ${H}" style="width:100%"><defs><linearGradient id="qg" x1="0%" y1="0%" x2="100%" y2="0%"><stop offset="0%" stop-color="#c026d3"/><stop offset="100%" stop-color="#3b82f6"/></linearGradient></defs><line x1="${pl}" y1="${pt}" x2="${pl}" y2="${H-pb}" stroke="var(--border2)" stroke-width="1"/><line x1="${pl}" y1="${H-pb}" x2="${W-pr}" y2="${H-pb}" stroke="var(--border2)" stroke-width="1"/>${labels}<path d="${path}" fill="none" stroke="url(#qg)" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"/>${dots}</svg><div style="font-size:10px;color:var(--text3);font-family:'DM Mono',monospace;text-align:center;margin-top:2px">x: quantum &nbsp;|&nbsp; y: avg wait (ms) &nbsp;|&nbsp; <span style="color:var(--mag2)">●</span> current</div></div>`; 
} 
function buildMQGantt(segs,gidPrefix){ 
  ...
  const rows=queues.map((q,qi)=>{ 
    const qsegs=segs.filter(s=>s.queue===q||s.id==='idle'); 
    // build a timeline for this queue — show process blocks where this queue ran, grey otherwise 
    const bars=segs.map(s=>{ 
      const w=((s.end-s.start)/total*100).toFixed(3); 
      ...
      return`<div class="gb" data-w="${w}" data-gid="${gidPrefix}-${qi}" style="background:${color};height:36px;border-right:2px solid rgba(0,0,0,.2)" title="P${s.id}: ${s.start}–${s.end} (${q})">P${s.id}</div>`; 
    }).join(''); 
  ...
  segs.filter(s=>s.id!=='idle'&&s.queue).forEach(s=>{if(!assigned[s.queue].includes(s.id))assigned[s.queue].push(s.id);}); 
  const assignRows=Object.entries(assigned).map(([q,ids])=>{ 
    ...
    return`<div style="display:flex;align-items:center;gap:8px;margin-bottom:5px"> 
      ...
      <span style="display:flex;gap:4px">${ids.map(id=>{const pidx=procs.findIndex(p=>p.id===id);return`<span style="font-size:10px;font-family:'DM Mono',monospace;padding:2px 7px;border-radius:4px;background:${COLORS[pidx%COLORS.length]}22;border:1px solid ${COLORS[pidx%COLORS.length]}44;color:${COLORS[pidx%COLORS.length]}">P${id}</span>`;}).join('')}</span> 
    </div>`; 
  }).join(''); 
...
function buildMFQGantt(segs,gidPrefix){ 
  ...
  const rows=levels.map((lv,li)=>{ 
    const bars=segs.map(s=>{ 
      ...
      const color=COLORS[ci(s.id)%COLORS.length]; 
      return`<div class="gb" data-w="${w}" data-gid="${gidPrefix}-${li}" style="background:${color};height:36px;border-right:2px solid rgba(0,0,0,.2)" title="P${s.id}: ${s.start}–${s.end} (L${lv})">P${s.id}</div>`; 
    }).join(''); 
    return`<div class="mrow"> 
  ...
  const demoteRows=Object.entries(demotions).map(([id,levels])=>{ 
    ...
    const badges=levels.map((lv,i)=>`${i>0?`<span class="demote-arrow"> → </span>`:''} 
      <span class="dbadge" style="background:${lbg[lv]};border:1px solid ${lc[lv]}44;color:${lc[lv]}">L${lv}</span>`).join(''); 
    const demoteCount=levels.length-1; 
    const note=demoteCount===0?'<span style="font-size:9px;color:var(--green);font-family:DM Mono,monospace"> ✓ finished L1</span>' 
...
function renderPlaybackState(tick, segs, procs, total) {
  ...
  // Calculate remaining bursts at the current tick
  segs.filter(s => s.id !== 'idle').forEach(s => {
    if (tick >= s.end) {
  ...
  const readyQueue = procs.filter(p => p.arr <= tick && remainingBursts[p.id] > 0 && (!running || running.id !== p.id));
  ...
  if (rqDisplay) {
    if (readyQueue.length === 0) {
      rqDisplay.innerHTML = '<span style="color:var(--text3)">empty</span>';
    } else {
      rqDisplay.innerHTML = readyQueue.map(p => {
  ...
  if (gwrap) {
    ...
    const pct = total === 0 ? 0 : (tick / total) * 100;
...
function animate(id){ 
  document.querySelectorAll(`#${id} .gb`).forEach((b,i)=>{ 
    b.style.width='0'; 
    setTimeout(()=>{b.style.transition='width 0.28s cubic-bezier(.4,0,.2,1)';b.style.width=b.dataset.w+'%';},i*80); 
  }); 
} 
function showOut(html){ 
  document.getElementById('empty').style.display='none'; 
  const o=document.getElementById('output');o.classList.remove('hidden');o.innerHTML=html; 
} 
function runActive(){ 
...
function runSingle(){ 
  const algo=document.getElementById('algo').value,q=+document.getElementById('quantum').value||2; 
  ...
  const NAMES={fcfs:'First Come, First Served',sjf:'Shortest Job First',srtf:'Shortest Remaining Time First',rr:`Round Robin — q=${q}`,priority:'Priority Scheduling',hrrn:'Highest Response Ratio Next',mq:'Multiple Queue Scheduling',mfq:'Multilevel Feedback Queue'}; 
  const BADGES={fcfs:'Non-preemptive',sjf:'Non-preemptive',srtf:'Preemptive',rr:'Preemptive',priority:'Non-preemptive',hrrn:'Non-preemptive',mq:'Multi-queue',mfq:'Preemptive'}; 
  const idleNote=idle>0?`<span style="font-size:10px;font-family:'DM Mono',monospace;color:var(--red2);margin-left:10px">⬤ idle: ${idle}ms</span>`:''; 
...
function runCompare(){ 
  ...
  const all=algos.map(a=>({...a,segs:getSegs(a.key,q)})).map(a=>({...a,st:computeStats(a.segs,procs)})); 
